...
N.B. This code acts a bit strangely on text with punctuation.
...
def romanize_word(word):
    ...
    # parse Hangul and convert sound-by-sound romanization
    for syllable in range(len(word)):
        ...
        if block_ord == 7000: # 의
            ...
            phoneme_list.append("x") # placeholder for non-Korean blocks
    ...
    for syllable in range(len(word) - 1):
        ...
        # palatalization  -  should only occur for 이, 히, 여, 혀 as grammatical particles, but oh well
        ...
        if phoneme_list[next_initial] == "h":
            if final_carry == "g":
                ...
                if show_h == 0:
                    # ㅎ-linking (not official, but natural)
    ...
    for syllable in range(len(word)):
        ...
        if phoneme_list[initial] == "x": # patch in non-Korean words by handling placeholder
...
print(romanize("고양이가 공허한 고향으로 방문했다"))             # ㅎ linking
